...
contract SoarCoinImplementationV02 is Owned {
    ...
    function SoarCoinImplementationV02(address _interface, SoarCoinImplementationV01 _previousImplementation, address _oracle) {
        _totalSupply = _previousImplementation.totalSupply();
        trustedContract = _interface;
    ...
    event EthForTokenFailed(address to, uint256 ethAmount);
    ...
    function ethForToken(address _participant, uint256 _amount) payable {
        ...
        if (transfer(_participant, oracle, _amount)) {
            ...
            EthForTokenFailed(_participant, msg.value);
